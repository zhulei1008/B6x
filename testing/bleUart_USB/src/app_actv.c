...
static void app_adv_set_adv_data(void)
{
    ...
    // Set list of UUIDs
    adv_data[0] = 0x03;
    adv_data[1] = GAP_AD_TYPE_COMPLETE_LIST_16_BIT_UUID; // 0x03
    #if (PRF_HOGPD)
    write16p(&adv_data[2], 0x1812); // HID Service
    #else
    write16p(&adv_data[2], 0xFF00); // Serial Service
    #endif
    length += 4;
    ...
    adv_data[4] = 0x03;
    adv_data[5] = GAP_AD_TYPE_APPEARANCE; // 0x19
    write16p(&adv_data[6], icon);
    length += 4;
