 ...
 * @{
 *
 *  Information about HyBle IP options and flags
 *
...
/// Kernel Message Heap
#define HYBLE_HEAP_MSG_SIZE         (  BT_HEAP_MSG_SIZE_      + \
                                      BLE_HEAP_MSG_SIZE_     + \
                                      BLEHL_HEAP_MSG_SIZE_      )

/// Size of Environment heap
#define HYBLE_HEAP_ENV_SIZE         ( BT_HEAP_ENV_SIZE_       + \
                                     BLE_HEAP_ENV_SIZE_      + \
                                     BLEHL_HEAP_ENV_SIZE_       )


/// Size of Attribute database heap
#define HYBLE_HEAP_DB_SIZE          (  BLEHL_HEAP_DB_SIZE_  )
 ...
 */
#define HYBLE_HEAP_NON_RET_SIZE    ( ECC_HEAP_NON_RET_SIZE_ )

/// Minimum sleep time to enter in deep sleep (in half slot).
#define HYBLE_MINIMUM_SLEEP_TIME                (1)
...
#define HYBLE_PTI_TXEN           1
///Disable TX busy signal
#define HYBLE_PTI_TXDIS          0
/// Tx busy position
#define HYBLE_TXBSY_POS          0

///Enable RX busy signal
#define HYBLE_PTI_RXEN           1
///Disable RX busy signal
#define HYBLE_PTI_RXDIS          0
/// Rx busy position
#define HYBLE_RXBSY_POS          1

///Enable do not abort TX
#define HYBLE_PTI_DNABORTEN      1
///Disable do not abort TX
#define HYBLE_PTI_DNABORTDIS     0
/// Do not abort busy position
#define HYBLE_DNABORT_POS        2

/// SAM disabled
#define HYBLE_SAM_DIS            0
/// SAM enabled
#define HYBLE_SAM_EN             1
/// SAM enable position
#define HYBLE_SAMEN_POS          3

/// Bit masking
#define HYBLE_COEX_BIT_MASK      1

/// Coex configuration index
enum hyble_coex_config_idx
{
    #if (BT_EMB_PRESENT)
    HYBLE_COEX_MSSWITCH_IDX,
    HYBLE_COEX_SNIFFATT_IDX,
    HYBLE_COEX_PAGE_IDX,
    HYBLE_COEX_PSCAN_IDX,
    HYBLE_COEX_INQ_IDX,
    HYBLE_COEX_INQRES_IDX,
    HYBLE_COEX_SCORSVD_IDX,
    HYBLE_COEX_BCAST_IDX,
    HYBLE_COEX_CONNECT_IDX,
    #endif //#if (BT_EMB_PRESENT)
    #if (BLE_EMB_PRESENT)
    HYBLE_COEX_CON_IDX,
    HYBLE_COEX_CON_DATA_IDX,
    HYBLE_COEX_ADV_IDX,
    HYBLE_COEX_SCAN_IDX,
    HYBLE_COEX_INIT_IDX,
...
enum hyble_prio_dft
{
    #if (BT_EMB_PRESENT)
    /// ACL event default priority
    HYBLE_PRIO_ACL_DFT               = 5,
    /// ACL event priority with activity
    HYBLE_PRIO_ACL_ACT               = 10,
    /// ACL Role Switch event default priority
    HYBLE_PRIO_ACL_RSW               = 20,
    /// ACL sniff event default priority
    HYBLE_PRIO_ACL_SNIFF_DFT         = 15,
    /// ACL sniff transition event default priority
    HYBLE_PRIO_ACL_SNIFF_TRANS       = 10,
    #if MAX_NB_SYNC
    /// SCO event default priority
    HYBLE_PRIO_SCO_DFT               = 18,
    #endif //MAX_NB_SYNC
    /// Broadcast ACL event default priority
    HYBLE_PRIO_BCST_DFT              = 5,
    /// Broadcast ACL event with LMP activity priority
    HYBLE_PRIO_BCST_ACT              = 10,
    /// CSB RX event default priority
    HYBLE_PRIO_CSB_RX_DFT            = 10,
    /// CSB TX event default priority
    HYBLE_PRIO_CSB_TX_DFT            = 10,
    /// Inquiry event default priority
    HYBLE_PRIO_INQ_DFT               = 5,
    /// Inquiry Scan event default priority
    HYBLE_PRIO_ISCAN_DFT             = 5,
    /// Page event default priority
    HYBLE_PRIO_PAGE_DFT              = 8,
    /// Page first packet event default priority
    HYBLE_PRIO_PAGE_1ST_PKT          = 20,
    /// PCA event default priority
    HYBLE_PRIO_PCA_DFT               = 20,
    /// Page scan event default priority
    HYBLE_PRIO_PSCAN_DFT             = 8,
    /// Page scan event priority increment when canceled
    HYBLE_PRIO_PSCAN_1ST_PKT         = 20,
    /// Synchronization Scan event default priority
    HYBLE_PRIO_SSCAN_DFT             = 10,
    /// Synchronization Train event default priority
    HYBLE_PRIO_STRAIN_DFT            = 10,
    #endif //#if (BT_EMB_PRESENT)
    #if (BLE_EMB_PRESENT)
    /// Default priority for scanning events
    HYBLE_PRIO_SCAN_DFT              = 5,
    /// Default priority for auxillary scan/init (no_asap) rx events
    HYBLE_PRIO_AUX_RX_DFT            = 12,
    /// Default priority for periodic adv rx events
    HYBLE_PRIO_PER_ADV_RX_DFT        = 16,
    /// Active priority for periodic adv rx events
    HYBLE_PRIO_INIT_DFT              = 10,
    /// LE connection events default priority
    HYBLE_PRIO_CONNECT_DFT           = 14,
    /// LE connection events priority with activity
    HYBLE_PRIO_CONNECT_ACT           = 16,
    /// Default priority for advertising events
    HYBLE_PRIO_ADV_DFT               = 5,
    /// Default priority for advertising high duty cycle events
    HYBLE_PRIO_ADV_HDC_DFT           = 10,
    /// Default priority for aux advertising events
    HYBLE_PRIO_ADV_AUX_DFT           = 12,
    /// Default priority for periodic advertising events
    HYBLE_PRIO_PER_ADV_DFT           = 10,
    /// Default priority for resolvable private addresses renewal event
    HYBLE_PRIO_RPA_RENEW_DFT         = 10,
    #endif // #if (BLE_EMB_PRESENT)
    /// Max priority
    HYBLE_PRIO_MAX                   = 31,
};
