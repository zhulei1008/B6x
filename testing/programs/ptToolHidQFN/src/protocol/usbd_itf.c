...
//    }
//    if (read_byte)
//    uart2_send(data, read_byte);
    
    if (usbd_rxd.head + read_byte <= RXD_BUFF_SIZE)
    {
        memcpy(&usbd_rxd.data[usbd_rxd.head], data, read_byte);
    }
    else
    {
        uint16_t lenEnd = RXD_BUFF_SIZE - usbd_rxd.head;
        memcpy(&usbd_rxd.data[usbd_rxd.head], data, lenEnd);
        memcpy(&usbd_rxd.data[0], &data[lenEnd], read_byte - lenEnd);
    }

    usbd_rxd.head = (usbd_rxd.head + read_byte) % RXD_BUFF_SIZE;     
}
...
//        uart2_send(cdc_bulk_buff, CDC_BULK_TX_SIZE);
