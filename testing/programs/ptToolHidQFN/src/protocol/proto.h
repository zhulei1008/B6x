...
enum PT_CMD_CODE
{
    ...
    PT_CMD_TEST_TRIM     = 0x46, // Test C LDO
};
...
#define SIZE_MAC_INFO      10  // sizeof(macInfo_t)
...
enum PAYL_LEN
{
    ...
    /* Proto: Sync */
    PLEN_CMD_INFO        = 0x00,  //
    PLEN_RSP_INFO        = 0x100,  // 2version + sdrvInfo_t + firmInfo_t + macInfo_t + 1crc mdf0806-6vip
    ...
    PLEN_CMD_CHRET       = 0x01,  //
    PLEN_RSP_CHRET       = 0x14,  // 10macret+8test+1chan+1crc(SIZE_MAC_INFO+SIZE_TEST_INFO+2)
    ...
    /* Proto: Test */
    PLEN_CMD_TRIMVAL     = 0x20,  // 0x80 & xo16m_cap_trim
    ...
    PLEN_CMD_TEST_TRIM   = 0x03,  // 1mode
    PLEN_RSP_TEST_TRIM   = 0x05,  // 1mode+4result
...
enum ft_cmd
{
    ...
    CMD_CORELDO_RUN_ADD         = 0x4c, // test ok add by door 2023.7.17, reason->coreldo > 1.2v 
    CMD_CORELDO_RUN_RECORD      = 0x52, // test ok add by door 2023.7.17, reason->record coreldo value
    CMD_CORELDO_DP_ADD          = 0x34, // test ok
    CMD_CORELDO_DP_REDUCE       = 0x4E, // test ok
    CMD_CORELDO_RECOVER         = 0x50, // test ok
    
    CMD_AONLDO_INIT             = 0x35, // test ok
    CMD_AONLDO_RUN_RIS_ADJ      = 0x36, // test ok
    CMD_AONLDO_RUN_FAL_ADJ      = 0x37, // test ok
    CMD_AONLDO_RUN_ADD          = 0x4d, // test ok add by door 2023.7.17 reason->aonldo > coreldo
    CMD_AONLDO_OFF_ADD          = 0x38, // test ok   
    CMD_AONLDO_OFF_REDUCE       = 0x4F, // test ok   
    CMD_AONLDO_RECOVER          = 0x51, // test ok
    
    CMD_RFBANDGAP_TEST          = 0x53, // test ok add by door 2023.7.21
	/**********************************************************************/
    CMD_TRIM_LOAD               = 0x3A, 
};
/***************************************************************************
...
struct pt_cmd_test_trim
{
    uint16_t val;    
    uint8_t ft_cmd;
};

struct pt_rsp_test_trim
{
    uint32_t result;    
    uint8_t ft_cmd;
};
...
// void pt_cmd_sync(void);
#define pt_cmd_info()       pt_cmd_noargs(PT_CMD_INFO)
