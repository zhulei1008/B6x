...
#define UART_IO_TX 6 // PA6
#endif
#if !defined(UART_IO_RX)
#define UART_IO_RX 7 // PA7
#endif

#if !defined(UART_BAUD)
#define UART_BAUD BRR_DIV(115200, 16M)
#endif
...
static void uartCmdPortInit(void)
{
    ...
    uart_conf(UART_CMD_PORT, UART_BAUD, LCR_BITS_DFLT);
}

#define WAIT_UART_IDLE() uart_wait(UART_CMD_PORT)
...
void userUartParse(void)
{
    ...
    while (1)
    {
        ...
        switch (frame->cmd)
        {
            case UART_CMD_RANDOM_NUM:
                if (waitFixedFrame(frame, BODY_LEN_RANDOM_NUM))
                {
                    uint32_t randnom_num = random_number_get();
                    uartRspData(UART_RSP_RANDOM_NUM, 4, (uint8_t *)&randnom_num);
                }
                break;
                
            case UART_CMD_TEMP:
                if (waitFixedFrame(frame, BODY_LEN_TEMP))
                {
                    uint16_t temp_val = sadc_temperature_get();
                    uartRspData(UART_RSP_TEMP, 2, (uint8_t *)&temp_val);
                }
                break;
            ...
            case UART_CMD_TX_GAIN_CAL:
                if (waitFixedFrame(frame, BODY_LEN_TX_GAIN_CAL))
                {
                    ...
                    uint16_t gain_res = rf_tx_gain_cal(pll_dis, pll_frach);

                    uartRspData(UART_RSP_TX_GAIN_CAL, 2, (uint8_t *)&gain_res);
                }
                break;
            ...
            case UART_CMD_DPLL_LOCK:
                ...
                {
                    uint8_t dpll_48m  = *(frame->body + 0);
                    uint8_t lock = dpll_384m_lock(dpll_48m);
            ...
            case UART_CMD_RC16M:
                if (waitFixedFrame(frame, BODY_LEN_RC16M))
                {
                    ...
                    uartRspData(UART_RSP_RC16M, BODY_LEN_RC16M, frame->body);
                }
                break;
...
//                    rc32k_test_cmd(ctl, spi_code);
            ...
            case UART_CMD_LDO_DIG:
                if (waitFixedFrame(frame, BODY_LEN_LDO_DIG))
                {
                    ...
                    rf_bandgap_test_cmd(bg_bias_trim, bg_vref_fine, bg_res_trim);
//                    
                    uartRspData(UART_RSP_LDO_DIG, BODY_LEN_LDO_DIG, frame->body);
                    
//                    uint16_t adc_val = sadc_rf_bandgap(bg_bias_trim, bg_vref_fine, bg_res_trim);
//                    
//                    uint16_t adc_val = sadc_vdd_if(bg_res_trim);
//                    uartRspData(UART_RSP_LDO_DIG, 2, (uint8_t *)&adc_val);
                }
                break;
            ...
            case UART_CMD_RSSI:
                if (waitFixedFrame(frame, 0))
                {
                    uint32_t rssi_val = rssi_get();
                    uartRspData(UART_RSP_RSSI, 4, (uint8_t *)&rssi_val);
                }
                break;
...
//                    uint8_t ret = get_pll_freq_adj_st(*(frame->body + 0));
                    uint8_t ret = 0;//get_gain_cal_st(*(frame->body + 0));
                    tx_sw_channel_num_test(*(frame->body + 0));
