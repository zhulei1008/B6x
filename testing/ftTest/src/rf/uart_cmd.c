...
{
    uartIOInit(UART_CMD_PORT, UART_IO_TX, UART_IO_RX);
    uart_param_t cfg = {115200, DATA_BITS_8b, STOP_BITS_1b, PARITY_NONE};
    uartConfig(UART_CMD_PORT, &cfg);
}
...
static void uartRspState(uint8_t rspId, uint8_t state)
{
    ...
    uartPutc(UART_CMD_PORT, UART_RSP_SYNC);
    uartPutc(UART_CMD_PORT, rspId);

    // frame.len=0x0001, LSB
    uartPutc(UART_CMD_PORT, 0x01); 
    uartPutc(UART_CMD_PORT, 0x00);

    uartPutc(UART_CMD_PORT, state);
}
...
static void uartRspData(uint8_t rspId, uint16_t len, const uint8_t *dat)
{
    ...
    uartPutc(UART_CMD_PORT, (uint8_t)(flen & 0xFF)); // frame.len, LSB
    uartPutc(UART_CMD_PORT, (uint8_t)(flen >> 8));

    for (i = 0; i < len; i++ )
    {
        uartPutc(UART_CMD_PORT, dat[i]);
        crc ^= dat[i];
    }
    uartPutc(UART_CMD_PORT, crc);
}
...
static bool waitFixedFrame(frame_t *pfm, uint16_t fixlen)
{
    ...
    for (i = 0; i < flen; i++)
    {
        pfm->body[i] = uartGetc(UART_CMD_PORT);
    }
...
void userUartParse(void)
{
    ...
    while (1)
    {
        ...
        buffer[recvlen++] = uartGetc(UART_CMD_PORT);
