 ...
 ****************************************************************************************
 */
extern uint8_t dongle_addr[8];
#if(_24G_PARING_ENABLE)
bool store_dongle_mac = 0;
#endif
APP_MSG_HANDLER(gapc_cmp_evt)
{
    // Command complete, may ignore
    if(param->operation == GAPC_UPDATE_PARAMS/*9*/)
    {
        updata_conn_status =  param->status;
    }
    
    DEBUG("CMP_EVT(op:%d,sta:0x%x)", param->operation, param->status);
}
extern const uint8_t  def_master_dongle_addr[];
...
//    #endif
    ...
    store_dongle_mac = 0;
    #endif
    DEBUG("gapc_connection_req_ind(cid:%d,chdl:%d,conn[intv:%d,late:%d,to:%d],caddr:%d)", conidx, param->conhdl,
          param->con_interval, param->con_latency, param->sup_to, param->peer_addr_type);
    ...
    if(g_sync_word == SYNC_WORD_2G4)
    {
        DEBUG("dongleaddr");
        debugHex(dongle_addr, 6);
        DEBUG("addr");
        debugHex(addr, 6);
        if(memcmp(dongle_addr,addr,6)==0)
        {
            DEBUG("debugfirst");
            memcpy(dongle_addr,param->peer_addr.addr,6);
            #if(_24G_PARING_ENABLE)
            store_dongle_mac = 1;
            #endif
        }
        else if(memcmp(dongle_addr,param->peer_addr.addr,6))
        {
            DEBUG("deff");
            //if(memcmp(def_master_dongle_addr,param->peer_addr.addr,6))
            gapc_disconnect(conidx);//disconnect;
        }
    }

    conn_intv = param->con_interval;
    conn_late = param->con_latency;
}
...
APP_MSG_HANDLER(gapc_connection_ind)
{
    ...
    if(store_dongle_mac)
    {
        flash_erase_write(DONGLE_MAC_OFFSET,(uint32_t*)&dongle_addr);
    }                  
    #endif
    ...
    switch (param->request)
    {
        ...
        case (GAPC_TK_EXCH):
        {
            ...
            else if (param->data.tk_type == GAP_TK_DISPLAY)
            {
                ...
                uint32_t pin_code = 123456;//(100000 + (rand_word() % 900000));
                // Set the TK value
