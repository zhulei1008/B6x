...
APP_MSG_HANDLER(gapc_connection_req_ind)
{
    ...
    {
        if (0 == memcmp((uint8_t *)param->peer_addr.addr, (uint8_t *)(user_scan_addr_list + i), GAP_BD_ADDR_LEN))
        {
            if (user_scan_addr_list[i].icon_type == ICON_KB)
            {
                g_cid_kb = conidx;
                //slave_type = g_cid_kb;
            }
    ...
    DEBUG("kid:%d, cid:%d", g_cid_kb, conidx);
...
APP_MSG_HANDLER(gapc_disconnect_ind)
{
    ...
    DEBUG("gapc_disconnect_ind(cid:%d,hdl:%d,reason:0x%X)\r\n", conidx, param->conhdl, param->reason);
