...
APP_MSG_HANDLER(gapc_connection_req_ind)
{
    ...
    memcpy(read_Master_Addr, param->peer_addr.addr, 6);
...
extern uint8_t ota_is_ok;
extern void ota_error_repair(void);
const uint8_t masterDongle_ota_Addr[] ={0x11, 0x88, 0x3F, 0xA1, 0x01, 0xF2};
APP_MSG_HANDLER(gapc_disconnect_ind)
{
    ...
    if ((memcmp(read_Master_Addr,masterDongle_ota_Addr, 6)==0)&&(ota_is_ok==0))
    {
        DEBUG("repair\r\n");
        ota_error_repair();
    }
