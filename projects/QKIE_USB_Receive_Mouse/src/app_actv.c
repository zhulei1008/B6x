...
struct gap_bdaddr scan_addr_list_back[SCAN_NUM_MAX];
   ...
   // DEBUG("Evt(op:0x%X,sta:0x%X)", gapm_op, status);
    ...
    switch (gapm_op)
    {
        ...
        case GAPM_STOP_ACTIVITY:
        {
            ...
            //DEBUG("-->Filter DevAddr:%d, sta:%d", scan_cnt, app_state_get());
//            
...
void app_scan_result(const struct gap_bdaddr* paddr)
{
    ...
    if (scan_cnt < SCAN_NUM_MAX) //get null array
    {
        ...
        debugHex((uint8_t *)&scan_addr_list_back[scan_cnt],7);
        debugHex((uint8_t*)paddr,7);
        if (memcmp((uint8_t *)&invalid_mac, (uint8_t *)&scan_addr_list_back[scan_cnt], 7)==0)
        {
        ...
        else if(memcmp(paddr, (uint8_t *)&scan_addr_list_back[scan_cnt], 7)==0)
        {
            memcpy(&scan_addr_list[scan_cnt], paddr,sizeof(struct gap_bdaddr));
            scan_cnt++;
            //DEBUG("FIND_OLD----------------");
...
//                if ((icon > GAP_APPEARANCE_GENERIC_HID) && (icon < GAP_APPEARANCE_HID_BARCODE_SCANNER))
//                {
...
//                }
            ...
            if ((icon == GAP_APPEARANCE_HID_MOUSE) && (ad_type == GAP_AD_TYPE_MANU_SPECIFIC_DATA) && (*p_cursor > 4) && (0 == memcmp(p_cursor+2, "QKIE", 4)))
            {
                DEBUG("---Find:%d", report->length);
                debugHex(report->data, report->length);
                app_scan_result(&report->trans_addr);
                break;
            }
//            if ((*p_cursor == 0x0F) && (ad_type == GAP_AD_TYPE_MANU_SPECIFIC_DATA))
//            {
//                DEBUG("---Find:%d", report->length);
//                debugHex(report->data, report->length);
//                app_scan_result(&report->trans_addr);
...
//{
//    .addr      = {{0x00, 0x31, 0x08, 0x23, 0x20, 0xD2}},
//    .addr_type = ADDR_RAND,
//};
...
//        }
        ...
        init_param.conn_to=300;//解决插入dongle 不连的问题
