 ...
 ****************************************************************************************
 */
struct gap_bdaddr read_slave_addr;
extern struct gap_bdaddr scan_addr_list_back[SCAN_NUM_MAX];
bool store_slave_addr_flag = 0;
...
APP_MSG_HANDLER(gapc_connection_req_ind)
{
    ...
    read_slave_addr.addr_type = param->peer_addr_type;
    memcpy((uint8_t*)&read_slave_addr, (uint8_t*)&param->peer_addr,6);
    app_scan_action(ACTV_STOP);
    init_timer_stop();
//    memset((uint8_t *)scan_addr_list, 0x00, (sizeof(struct gap_bdaddr) * SCAN_NUM_MAX));
//    scan_cnt_back = scan_cnt;
//    scan_cnt = 0;
}
...
APP_MSG_HANDLER(gapc_connection_ind)
{
    ...
    struct gap_bdaddr invalid_mac_gapc = {{0}, 0};
    if (memcmp((uint8_t *)&invalid_mac_gapc, (uint8_t *)&scan_addr_list_back[scan_cnt-1], 7)==0)
    {
        store_slave_addr_flag=1;
    }
...
APP_MSG_HANDLER(gapc_disconnect_ind)
{
    ...
    memset((uint8_t *)scan_addr_list, 0x00, (sizeof(struct gap_bdaddr) * SCAN_NUM_MAX));
    #if(FAST_DONGLE_MODE)
    memset((uint8_t *)scan_addr_list_back, 0x00, (sizeof(struct gap_bdaddr) * SCAN_NUM_MAX));
    #endif
    scan_cnt--;
