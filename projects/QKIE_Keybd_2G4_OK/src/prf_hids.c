...
uint8_t Request_connection_parameters_cont;
...
const att_decl_t hid_atts[] =
{
    ...
    #if (HID_RPT_SYSTEM)
    // Sleep IN Report Char. Declaration and Value, Report Ref. and CCC Descriptor
...
static uint8_t hids_rpt_ntf_cfg(uint8_t conidx, uint8_t rpt_idx, const struct atts_write_ind *ind)
{
    ...
    if ((!ind->more) && (ind->length == sizeof(uint16_t)))
    {
        ...
        if (cli_cfg <= PRF_CLI_START_NTF)
        {
            ...
            status = LE_SUCCESS;
            if(AON->BACKUP1    == 3)
            ke_timer_set(HOGPD_WAKEUP_REPORT,TASK_APP,2000);    
        }
    }
...
#if (CFG_2G4_MODE)
    // 2.4g模式无须加密
    hid_svc_db.info = SVC_UUID(16) | SVC_SEC(UNAUTH);
    if (work_mode == B24G_MODE)
    {
        hid_svc_db.info = SVC_UUID(16);
    }
#endif //CFG_2G4_MODE
