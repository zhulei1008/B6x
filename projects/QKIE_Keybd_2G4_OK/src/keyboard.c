...
const uint8_t Key_Map[KEY_ROW_NB][KEY_COL_NB] =
{
    {KEY_F5,   KEY_1,      KEY_2,         KEY_3,  KEY_4, KEY_7, KEY_9,      KEY_0,         KEY_8,        0x00    , KEY_F12,  0x00,      KEY_F10,      0x00,        KEY_RIGHT,  0x00,        },
    {KEY_FN,   KEY_ACCENT, KEY_F1,        KEY_F2, KEY_5, KEY_6, KEY_F8,     KEY_MINUS,     KEY_EQUAL,    0x00    , 0x00,     KEY_TAB,   KEY_F9,       0x00,        KEY_RIGHT,  KEY_DELETE,  },
    {0x00,     0x00,       0x00,          0x00,   KEY_B, KEY_N, 0x00,       KEY_SLASH,     0x00,         0x00    , KEY_RGUI, KEY_LEFT,  0x00,         0x00,        KEY_RIGHT,  KEY_DOWN,    },
    {KEY_RALT, KEY_Z,      KEY_X,         KEY_C,  KEY_V, KEY_M, KEY_PERIOD, 0x00,          KEY_COMMA,    0x00    , 0x00,     KEY_S,     KEY_ENTER,    0x00,        KEY_RIGHT,  KEY_F11,     },
    {0x00,     KEY_ESC,    0x00,          KEY_F4, KEY_G, KEY_H, 0x00,       KEY_QUOTE,     KEY_F6,       0x00    , KEY_LGUI, KEY_UP,    0x00,         0x00,        KEY_RIGHT,  KEY_SPACEBAR,},
    {0x00,     KEY_A,      KEY_S,         KEY_D,  KEY_F, KEY_J, KEY_L,      KEY_SEMICOLON, KEY_K,        0x00    , KEY_LCTRL,0x00,      KEY_BACKSLASH,KEY_RSHIFT,  KEY_RIGHT,  0x00,        },
    {0x00,     KEY_TAB,    KEY_CAPS_LOCK, KEY_F3, KEY_T, KEY_Y, KEY_F7,     KEY_LBRACKET,  KEY_RBRACKET, KEY_LALT, 0x00,     0x00,      KEY_BACKSPACE,KEY_LSHIFT,  KEY_RIGHT,  0x00,        },
    {0x00,     KEY_Q,      KEY_W,         KEY_E,  KEY_R, KEY_U, KEY_O,      KEY_P,         KEY_I,        0x00    , 0x00,     0x00,      0x00,         0x00,        KEY_RIGHT,  0x00,        }
   //C0        C1          C2             C3      C4     C5     C7          C8             C6            C15       C9        C13        C10           C14          C12         C11           
};
#if (0)
...
#if (SYS_CLK == 1)
    #define DLY_US_PSC (32)
#elif (SYS_CLK == 2)
    #define DLY_US_PSC (48)
#elif (SYS_CLK == 3)
    #define DLY_US_PSC (64)
#else
    #define DLY_US_PSC (16)
#endif //SYS_CLK

#define DELAY_US(time) btmr_delay(DLY_US_PSC, (time))
/*
 * FUNCTIONS
 ****************************************************************************************
 */
...
static void col_scan(uint8_t col)
{
    ...
    for (uint8_t row = 0; row < KEY_ROW_NB; row++)
    {
        ...
        {
            DEBUG("(r:%d,c:%d,kcode:%x)", row, col, Key_Map[row][col]);
...
static void kb_send_report(uint8_t type, const uint8_t *report)
{
    ...
    #if (CFG_2G4_MODE)
    if ((app_state_get() < (APP_ENCRYPTED - work_mode))||(sys_poweroff_flag>0))    
    #else
...
//            gapc_key_press_notif_cmd(TASK_ID(GAPC, app_env.curidx), num);
    ...
    {             
        ...
        DEBUG("key_status_cont:%d, flag:%d",key_status_cont, update_connect_par_flag);
...
void kb_scan(void)
{
    ...
    for (uint8_t c = 0; c < KEY_COL_NB; c++)
    {
        ...
        {
            GPIO->DIR_SET = 0x1<<(Key_Col[c]);
            GPIO->DAT_CLR = 0x1<<(Key_Col[c]);
            DELAY_US(10);
        }
        ...
        {
            GPIO->DIR_CLR = 0x1 << (Key_Col[c]);
            //iom_ctrl(Key_Col[col],/*IOM_PULLUP|*/IE_UP);
            iom_ctrl(Key_Col[c],IOM_HIZ);
        }
        #endif
    }
