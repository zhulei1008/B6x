...
struct actv_env_tag actv_env; // Activities environment
...
static void app_adv_create(void)
{
    ...
    if (powerOn_Work_Status == SYS_IDLE)
    {
        ...
        memcpy(&adv_param.peer_addr.addr.addr[0], masterDongle_Addr, GAP_BD_ADDR_LEN);
    ...
    adv_param.prim_cfg.phy          = GAP_PHY_LE_1MBPS;
    adv_param.prim_cfg.chnl_map     = APP_ADV_CHMAP;
    adv_param.prim_cfg.adv_intv_min = APP_ADV_FAST_INT; // APP_ADV_INT_MIN;
    adv_param.prim_cfg.adv_intv_max = APP_ADV_FAST_INT; // APP_ADV_INT_MAX;
...
#endif
    {
        name_len=app_name_get(BT_NAME_MAX_LEN,&adv_data[length+2]);
...
static void app_adv_set_scan_rsp(void)
{
    ...
    {
        uint8_t length=0;
        uint8_t rsp_data[BT_NAME_MAX_LEN+8];
        uint8_t name_len = 0;
        // Set device name
//        length      = app_name_get(BT_NAME_MAX_LEN, &rsp_data[2]);
//        DEBUG("length:%d",length);
//        rsp_data[0] = length + 1;
//        rsp_data[1] = GAP_AD_TYPE_COMPLETE_NAME; // 0x09

        //sizeof(BLE_DEV_NAME_CHN_1)-1;//app_name_get();//app_dev_name(&rsp_data[2]);
        rsp_data[0] = 7;
        rsp_data[1] = GAP_AD_TYPE_MANU_SPECIFIC_DATA;
        rsp_data[2] = 'Q';
        rsp_data[3] = 'K';
        rsp_data[4] = 'I';
        rsp_data[5] = 'E';
        ...
        length += 8;
        DEBUG("length:%d\r\n",length);
        name_len = app_name_get(BT_NAME_MAX_LEN,&rsp_data[10]);
        DEBUG("name_len:%d\r\n",name_len);
        // Set device name
        rsp_data[8] = name_len + 1;
        rsp_data[9] = GAP_AD_TYPE_COMPLETE_NAME; // 0x09    
        
        length+=name_len;
        DEBUG("length:%d\r\n",length);
        gapm_set_adv_data(actv_env.advidx, GAPM_SET_SCAN_RSP_DATA, length + 2, rsp_data);
        debugHex(rsp_data,length+2);
    }
    DEBUG("SCAN_RSP_DATA");
}
