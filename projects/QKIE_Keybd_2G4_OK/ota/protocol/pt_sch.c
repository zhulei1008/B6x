...
static void proto_received(proto_t *pt)
{
    ...
    switch (pt->rx_state)
    {
        ...
        case PT_STATE_PAYL:
        {
            ...
            if ((pt->rx_pfmt & PKT_CRC_MSK)
                && (pkt_crc8(pkt->payl, pkt->len) != 0))
            {
                pt->rx_pfmt = PT_ERR_CRC; // CRC error
            }
        } break;
...
#if (PT_TOUT_CNT)
extern uint32_t currTickCnt(void);
...
static void proto_timeout(proto_t *pt)
{
    ...
    // calc timeout //zhushuai 
    if ((uint32_t)(sftmr_tick() - pt->rx_tick) > PT_TOUT_CNT)
    {
...
#if (PT_TOUT_CNT)
            pt->rx_tick = sftmr_tick();//zhushuai
        }
        else
        {
            proto_timeout(pt);
