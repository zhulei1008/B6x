...
void core_pwroff(uint16_t cfg_wkup)
{
    ...
    // enter poweroff, FixBug 6vp 1028 - core1.2v with 1uF(need 1.75S to enter), try to pwroff again if except
    while (1)
    {
...
__attribute__((section("ram_func.core.765")))
void core_unique_id(uint8_t size, uint8_t *unique_id)
{
    ...
    #else
    uint32_t rdcnt = 0;
    // wait last cmd finish
    while (FSHC->ST.CMD_SEND_STATUS);

    // config new command
    FSHC->CMD_REG                 = 0x4B; // Read Unique ID CMD
    FSHC->ADDR_REG                = 0;
    FSHC->RXPTR_INC_EN            = 1; // read-fifo enable
    FSHC->RXTX_DAT_LEN            = 15;// len - 1;
    FSHC->SEND_CTRL.Word          = 0x0395; // cmd(1B)+addr(3B)+dummy(1B) = cmd(1B)+dummy(4B)
    FSHC->MCU_ADRCMD_BIT_LEN.Word = 0x01DF;
    ...
    rdcnt = size > 16 ? 16 : size;
    xmemcpy(unique_id, (uint8_t *)tmp_id, rdcnt);
    #endif
    // ascii"B6x-"
    tmp_id[0] = 0x2D783642;
